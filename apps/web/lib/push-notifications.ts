...
export function usePushNotifications(vapidPublicKey: string) {
  const manager = new PushNotificationManager({ vapidPublicKey })
  ...
  const subscribe = async (userId: string) => {
    setLoading(true)
    try {
      const subscription = await manager.subscribe(userId)
  ...
  const testNotification = async (data: PushNotificationData) => {
    return manager.testNotification(data)
  }
